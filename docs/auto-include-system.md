...
Wir schauen uns dazu als erstes die fiktive Controller-Datei `example_xyz.php` an. Hier stellen wir jetzt fest, das es einen Autoinclude-Eintrag in Zeile 8 gibt, der fast immer wie folgt aussieht:
...
Wenn in einem Verzeichnis mehre Autoinclude-Dateien vorhanden sind, werden die Dateien in alphabetischer Reihenfolge geladen.
...
Im Wiki von modified gibt es mitlerweile ebenfalls einen Eintrag, der versucht alle Autoincludes aufzuführen (https://www.modified-shop.org/wiki/Auto_include_Modul_System). Falls du eine sehr neue modified Version verwendest, werden hier jedoch nicht immer ganz aktuell alle Autoincludes aufgelistet.
